...
static const char *level_char(device_log_level_t level)
{
    switch (level) {
...
void device_log(device_log_level_t level, const char *tag, const char *fmt, ...)
{
    ...
    switch (level) {
        ...
        case LOG_LEVEL_INFO:  ESP_LOGI(tag, "%s", msg); break;
        case LOG_LEVEL_DEBUG: ESP_LOGD(tag, "%s", msg); break;
    }
    ...
    int line_len = snprintf(line, sizeof(line), "[%lu][%s][%s] %s\n",
                            (unsigned long)uptime_ms, level_char(level), tag, msg);
    if (line_len <= 0) return;
    if (line_len >= (int)sizeof(line)) line_len = sizeof(line) - 1;

    xSemaphoreTake(s_mutex, portMAX_DELAY);
    if (s_ring_len + line_len < LOG_RING_SIZE) {
        memcpy(s_ring + s_ring_len, line, line_len);
        s_ring_len += line_len;
    }
