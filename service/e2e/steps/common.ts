...
import {
	assertCanvasHasColors,
	assertWebGLCanvasRendered,
} from "../support/canvas-utils";
import type { PlaywrightWorld } from "../support/world";

// ─── Viewport ──────────────────────────────────────────

Given(
	"I am using a {word} viewport",
	async function (this: PlaywrightWorld, viewport: string) {
...
When(
	"I click {string} in the navigation bar",
	async function (this: PlaywrightWorld, linkText: string) {
		if (this.viewport === "mobile") {
			...
			await this.page
				.locator(`[data-testid="mobile-menu"] a:has-text("${linkText}")`)
				.click();
		} else {
...
Then(
	...
	async function (this: PlaywrightWorld, selector: string) {
...
Then(
	...
	async function (this: PlaywrightWorld, selector: string, text: string) {
		await expect(this.page.locator(selector).first()).toContainText(text);
	},
);

Then(
	"I should see a link {string} to {string}",
	async function (this: PlaywrightWorld, text: string, href: string) {
		await expect(
			this.page.locator(`a[href="${href}"]`).filter({ hasText: text }).first(),
		).toBeVisible();
	},
);
...
Then(
	"I should be on the {word} page",
	async function (this: PlaywrightWorld, pageName: string) {
...
Then(
	"the element {string} should have attribute {string} with value {string}",
	async function (
		...
		await expect(this.page.locator(selector).first()).toHaveAttribute(
			attr,
			value,
		);
	},
...
When(
	...
	async function (this: PlaywrightWorld, text: string, name: string) {
...
When(
	...
	async function (this: PlaywrightWorld, option: string, name: string) {
		await this.page
...
When(
	"I wait for a response to {string}",
	async function (this: PlaywrightWorld, path: string) {
...
When(
	"I wait for the element {string} to have class {string}",
...
Then(
	"the canvas previews should render colors",
	async function (this: PlaywrightWorld) {
		await this.page.waitForFunction(
			() =>
				document.querySelectorAll("[data-preview-card].preview-ready").length >
				0,
			{},
			{ timeout: 30000 },
		);
		...
		const cards = await this.page
			.locator("[data-preview-card].preview-ready")
			.all();
		...
		for (const card of cards) {
			...
			const result = await assertCanvasHasColors(
				this.page,
				`[data-preview-card].preview-ready canvas`,
			);
			if (result.hasColors) {
				anyHasColors = true;
				break;
...
Then(
	"the WebGL viewer should render content",
	async function (this: PlaywrightWorld) {
